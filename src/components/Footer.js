import React from "react";
import "./Allcss.css";
const Footer = () => {
  return (
    <div>
      <div className="review">
        <p1>Customer Reviews</p1>
      </div>
      <div class="container-fluid">
        <div class="slider-container">
          <div class="review-bar">
            <i
              class="fas fa-chevron-circle-left slider-controls"
              id="prevBtn"
            ></i>

            <div class="reviewContent">
              <h3>analosem </h3>
              <h4>Lorem ipsum dolor sit amet.</h4>

              <div class="rating-container"></div>
              <h6>"****"</h6>
            </div>

            <i
              class="fas fa-chevron-circle-right slider-controls"
              id="nextBtn"
            ></i>
          </div>
        </div>
      </div>
      <div className="email">
        <h1 id="hp">Subscribe to our News Letter</h1>

        <div class="input-group mb-3" id="gg">
          <input
            type="text"
            class="form-control"
            placeholder="Type your E-Mail here..!"
            aria-label="Recipient's username"
            aria-describedby="button-addon2"
          />

          <button
            class="btn btn-outline-secondary"
            type="button"
            id="button-addon2"
          >
            GO
          </button>
        </div>
      </div>
      <section id = "allfoot">
      <footer id="footer" class="footer-1">
<div class="main-footer widgets-dark typo-light">
<div class="container">
<div class="row">
  
<div class="col-xs-12 col-sm-6 col-md-3">
<div class="widget subscribe no-box">
<h5 class="widget-title">Company Name<span>CLOUD KITCHEN </span></h5>
<br/>
<br/>
<p> No chef, Don't worry we are here for you..!</p>
</div>
</div>

<div class="col-xs-12 col-sm-6 col-md-3">
<div class="widget no-box">
<h5 class="widget-title">Quick Links<span></span></h5>
<ul class="thumbnail-widget">
<li>
<div class="thumb-content"><a href="#.">Get Started</a></div>	
</li>
<li>
<div class="thumb-content"><a href="#.">Top Leaders</a></div>	
</li>
<li>
<div class="thumb-content"><a href="#.">Success Stories</a></div>	
</li>
<li>
<div class="thumb-content"><a href="#.">Event/Tickets</a></div>	
</li>
<li>
<div class="thumb-content"><a href="#.">News</a></div>	
</li>
<li>
<div class="thumb-content"><a href="#.">Lifestyle</a></div>	
</li>
<li>
<div class="thumb-content"><a href="#.">About</a></div>	
</li>
</ul>
</div>
</div>

<div class="col-xs-12 col-sm-6 col-md-3">
<div class="widget no-box">
<h5 class="widget-title">Get Started with Cloud Kitchen<span></span></h5>
<p>Here you will find everything </p>
<a class="btn" href="/" target="_blank">Subscribe Us </a>
</div>
</div>

<div class="col-xs-12 col-sm-6 col-md-3">

<div class="widget no-box">
<h5 class="widget-title">Contact Us<span>Mob:-7489260831</span></h5>
  <br/>
<p><a href="mailto:sp@cse.jgec.ac.in" title="glorythemes"></a></p>
<ul class="social-footer2">
<li class=""><a title="youtube" target="_blank" href="/"><img alt="youtube" width="30" height="30" src="data:image/png;base64,"/></a></li>
<li class=""><a href="/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"/></a></li>
<li class=""><a href="/" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"/></a></li>
<li class=""><a title="instagram" target="_blank" href="https:/"><img alt="instagram" width="30" height="30" src="data:image/png;base64,"/></a></li>
</ul>
</div>
</div>
 
</div>
</div>
</div>
  
<div class="footer-copyright">
<div class="container">
<div class="row">
<div class="col-md-12 text-center">
<p>MADE WITH ❤️BY SURAJ PRASAD</p>
</div>
</div>
</div>
</div>
</footer>
</section>


    </div>
  );
};

export default Footer;
